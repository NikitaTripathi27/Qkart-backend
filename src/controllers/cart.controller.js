...
const updateProductInCart = catchAsync(async (req, res) => {
  if(req.body.quantity > 0){
    const updatedProduct = await cartService.updateProductInCart(req.user , req.body.productId , req.body.quantity)
    res.status(httpStatus.OK).send(updatedProduct);
  }
  else if(req.body.quantity === 0){
    const deletedProduct = await cartService.deleteProductFromCart(req.user ,req.body.productId , req.body.quantity)
...
const checkout = catchAsync(async (req, res) => {
  const newuser = await cartService.checkout(req.user);
    res.status(httpStatus.NO_CONTENT).send(newuser)
});
